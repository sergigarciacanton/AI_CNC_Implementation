...
from scheduling_config import MODEL_PATH, SEED, DQN_LOG_LEVEL, DQN_LOG_FILE_NAME, DIVISION_FACTOR, \
...
class DQNAgent:
    ...
    def get_action_space(self, obs):
        # Get those actions that are feasible: just useful positions
        action_space = []
        for a in range(1, self.env.hyperperiod * DIVISION_FACTOR + 1):
            if obs[a] == 1:
    ...
    def evaluate(self, eval_env: gym.Env, n_episodes: int = 50) -> List[pd.DataFrame]:
        ...
        for episode in range(n_episodes):
            ...
            while not done:
                ...
                well_done_steps += info['well_done']
                obs = next_obs
        ...
        self.logger.info(f'[I] Total steps: {total_steps}')
        self.logger.info(f'[I] Well done steps: {well_done_steps}')
