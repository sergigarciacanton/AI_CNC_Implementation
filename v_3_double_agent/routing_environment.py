...
from routing_vnf_generator import VNF
import numpy as np
from typing import Tuple, Dict, Any
from routing_config import ENV_LOG_LEVEL, ENV_LOG_FILE_NAME, TIMESTEPS_LIMIT, SLOT_CAPACITY, DIVISION_FACTOR, TRAINING_IF, \
...
class EnvironmentTSN(gym.Env):
    ...
    # Called during step
    def reward_function(self, next_node):
        if next_node < len(self.edges_slots):
            ...
            self.routing_reward -= 10 * cur_len
    ...
    def step(self, next_node) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if next_node >= len(self.graph.nodes):
            ...
            edge_positions = self.get_edge_positions((self.current_node, next_node))
            position_id = edge_positions.index(max(edge_positions))
            ...
            exit_code = self.reward_function(next_node)
...
# print(information)
# print(information)
# observation2, reward, terminated, trunc, information2 = env.step(2)
# print('Edges info:', env.edges_slots)
