...
from config import MODEL_PATH, SEED, DIVISION_FACTOR, TRAINING_EDGES_A, DQN_LOG_LEVEL, DQN_LOG_FILE_NAME, \
...
def save_evaluation(evaluation_num: int, obs: np.ndarray, reward: float, done: bool, step: int) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: Updated DataFrame with the cent evaluation result.
    """
    ...
    # Update the DataFrame with the cent row
    df.loc[step] = result

    # Now df contains the cent evaluation result
    return df
...
class DQNAgentA:
    ...
    def __init__(
        ...
        obs_space = env.observation_space_A.n
        action_space = env.action_space_A.n
        ...
        # Logging settings
        self.logger = logging.getLogger('dqn_A')
    ...
    def _plot(step: int, max_steps: int, scores: List[float], mean_scores: List[float], losses: List[float],
        ...
        if SAVE_PLOTS and step == max_steps:
            ...
            plt.savefig(PLOTS_PATH + 'plot_' + date + '_' + str(int(np.mean(scores[-100:]))) + '_A.png')
    ...
    def train(self, max_steps: int, plotting_interval: int = 1000, evaluation_interval: int = 10000,
        ...
        if episodes_count > 1000:
            date = datetime.now(tz=timezone(timedelta(hours=2))).strftime('%Y%m%d%H%M')
            model_name = 'model_' + date + '_' + str(int(np.mean(scores[-100:]))) + '_A.pt'
